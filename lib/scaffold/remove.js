...
/**
 * Will remove a service from bcccore-node.json
...
/**
 * Will remove the Node.js service and from the bcccore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The bcccore-node configuration directory
...
function remove(options, done) {
  ...
  var bcccoreConfigPath = path.resolve(configPath, 'bcccore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(bcccoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a bcccore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to bcccore-node.json
        removeConfig(bcccoreConfigPath, service, next);
      });
    }, done
  );
}
