...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 ...
                 'There are changes necessary in both bitcoin.conf and bcccore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/owstack/bcccore-node/blob/bitcoind/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed bcccore-node.json configuration file
 ...
 * @param {String} options.config.network - 'livenet/bcc', 'testnet/bcc' or 'regtest
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './bcccore-node.json');
...
function checkService(service) {
  ...
  if (!service.module.prototype ||
    ...
    throw new Error(
      'Could not load service "' + service.moduleName + '" as it does not support necessary methods and properties.'
    );
  }
}
...
function loadModule(req, service) {
  try {
    // first try in the built-in bcccore-node services directory
    ...
    if (servicePackage.bcccoreNode) {
      serviceModule = service.moduleName + '/' + servicePackage.bcccoreNode;
    }
...
function setupServices(req, servicesPath, config) {
  ...
  if (config.services) {
    for (var i = 0; i < config.services.length; i++) {
      ...
      service.moduleName = serviceHelper.mapName(service.name);
 ...
 * @param {Node} node - The Bcccore Node instance
 */
