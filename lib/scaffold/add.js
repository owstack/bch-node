...
function add(options, done) {
  ...
  var bchConfigPath = path.resolve(configPath, 'bch-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(bchConfigPath) || !fs.existsSync(packagePath)) {
    return done(
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      addService(configPath, service, function(err) {
        ...
        addConfig(bchConfigPath, serviceName, next);
      });
    }, done
  );
}
