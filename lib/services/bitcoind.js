...
Bitcoin.prototype.subscribeAddress = function(emitter, addresses) {
  ...
  for(var i = 0; i < addresses.length; i++) {
    if (bcccore.Address.isValid(addresses[i], this.node.network)) {
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of bcccore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of bcccore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of bcccore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to bitcoind from bcccore. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that bitcoind is undergoing a reindex. ' +
             ...
             'of bcccore-node services will start.');
    node._reindex = true;
  }
};
...
Bitcoin.prototype._getDefaultConf = function() {
  ...
  if (this.node.network === bcccore.Networks.testnet) {
...
Bitcoin.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = bcccore.crypto.Hash.sha256sha256(message);
 ...
 * @param {String|Address|Array} addressArg - An address string, bcccore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
...
Bitcoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  ...
  function transformUnspentOutput(delta) {
    var script = bcccore.Script.fromAddress(delta.address);
    return {
...
Bitcoin.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    ...
    if (clone) {
      // reverse start and end as the order in bcccore is most recent to less recent
...
Bitcoin.prototype._getAddressStrings = function(addresses) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof bcccore.Address) {
...
/**
 * Will retrieve a block as a Bcccore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = bcccore.Block.fromString(response.result);
...
/**
 * Will get a transaction as a Bcccore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
